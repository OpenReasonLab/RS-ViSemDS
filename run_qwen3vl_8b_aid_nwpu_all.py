...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run Qwen3-VL-8B zero-shot, random few-shot, and global RemoteCLIP "
    ...
    parser.add_argument(
        "--model",
        default=os.environ.get("QWEN3VL_8B_MODEL", "/root/autodl-tmp/models/Qwen3-VL-8B"),
    )
    parser.add_argument("--out-root", default="Qwen3-VL-8B/results_eval100_seed42")
    ...
    for dataset in args.datasets:
        ...
        if not args.skip_zero:
            command = [
                ...
                *base_backend_args,
                "--out-dir", f"{args.out_root}/qwen3vl_8b_zero_{dataset}",
            ]
        ...
        for shot in args.shots:
            ...
            if not args.skip_random:
                ...
                command = [
                    ...
                    *base_backend_args,
                    "--out-dir", f"{args.out_root}/qwen3vl_8b_random_{dataset}_shot{shot}",
                ]
                run_command(command, args.dry_run)
            ...
            if not args.skip_knn:
                ...
                command = [
                    ...
                    *base_backend_args,
                    "--out-dir", f"{args.out_root}/qwen3vl_8b_knn_{dataset}_shot{shot}",
                ]
                run_command(command, args.dry_run)

    print("\nQwen3-VL-8B suite finished.", flush=True)
