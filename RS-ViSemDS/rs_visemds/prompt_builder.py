...
def category_rules_sha256(
    dataset: str,
    class_order: list[str],
    prompt_mode: str = "paper_v1",
) -> str:
    if prompt_mode != "paper_v1":
        raise ValueError("Formal RS-ViSemDS uses paper_v1")
    payload = {
...
def prompt_template_sha256(
    ...
    payload = {
        ...
        "task_text": task_text(dataset, class_order, example_count, prompt_mode),
        "example_blocks": [
            (
        ...
        "output_instruction": output_instruction(class_order, prompt_mode),
        "prompt_mode": prompt_mode,
    }
    return _sha256_payload(payload)
