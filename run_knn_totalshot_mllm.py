...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Evaluate query-specific global RemoteCLIP-kNN total-shot MLLM prompts."
    )
...
def build_task_prompt(dataset: str, class_order: list[str], shot: int, prompt_mode: str) -> str:
    prompt = (
        ...
        f"The prompt contains {shot} labeled example image(s). They are the globally most similar "
        "support images retrieved for this target by a frozen RemoteCLIP visual encoder, ordered "
        "from highest to lowest cosine similarity. Classes may repeat and need not cover all "
        "candidate classes.\n\n"
    )
...
def build_content(
    ...
    content = [{
        "type": "text",
        "text": build_task_prompt(dataset, class_order, len(examples), prompt_mode),
    }]
...
def load_and_validate_examples(
    ...
    if len(shots) != 1 or strategies != {"knn"}:
        raise ValueError(f"Expected one kNN shot value, got shots={shots}, strategies={strategies}")
    ...
    for target in evaluation:
        ...
        if len(paths) != len(set(paths)):
            raise ValueError(f"Duplicate kNN example for target {target['target_id']}")
        scores = [float(row["score"]) for row in selected]
        if any(scores[index] < scores[index + 1] for index in range(len(scores) - 1)):
            raise ValueError(f"kNN scores are not descending for target {target['target_id']}")
...
def main():
    ...
    if not retrieval_config_path.exists():
        raise FileNotFoundError(f"Missing retrieval config: {retrieval_config_path}")
    retrieval_config = read_json(retrieval_config_path)
    required_retrieval = {
        "strategy": "knn",
        "feature_backend": "remoteclip",
        "encoder": "RemoteCLIP-ViT-B-32",
        "similarity": "cosine_similarity",
        "l2_normalized_embeddings": True,
        "scope": "global_support_pool",
        "nested_top_k": True,
    }
    for key, expected in required_retrieval.items():
        if retrieval_config.get(key) != expected:
            raise ValueError(
    ...
    if shot_definition not in (None, "total_examples"):
        raise ValueError(
            f"Retrieval configuration is not total-shot: "
            f"shot_definition={shot_definition!r}"
        )
    ...
    if args.resume and config_path.exists():
        ...
        for key in (
            ...
            "examples_sha256", "retrieval_config_sha256",
        ):
    ...
    for target in evaluation:
        ...
        row = {
            "dataset": summary["dataset"], "strategy": "knn", "shot": shot,
            ...
            "example_scores": "|".join(example["score"] for example in examples),
    ...
    print(f"Wrote global RemoteCLIP-kNN few-shot MLLM results to {out_dir}")
