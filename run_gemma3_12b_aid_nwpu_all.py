...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run Gemma 3-12B zero-shot, random few-shot, and global RemoteCLIP "
    ...
    parser.add_argument(
        "--model",
        default=os.environ.get("GEMMA3_12B_MODEL", "/root/autodl-tmp/models/Gemma 3-12B"),
    )
    parser.add_argument("--out-root", default="Gemma 3-12B/results_eval100_seed42")
    ...
    for dataset in args.datasets:
        ...
        if not args.skip_zero:
            command = [
                ...
                *base_backend_args,
                "--out-dir", f"{args.out_root}/gemma3_12b_zero_{dataset}",
            ]
        ...
        for shot in args.shots:
            ...
            if not args.skip_random:
                ...
                command = [
                    ...
                    *base_backend_args,
                    "--out-dir", f"{args.out_root}/gemma3_12b_random_{dataset}_shot{shot}",
                ]
                run_command(command, args.dry_run)
            ...
            if not args.skip_knn:
                ...
                command = [
                    ...
                    *base_backend_args,
                    "--out-dir", f"{args.out_root}/gemma3_12b_knn_{dataset}_shot{shot}",
                ]
                run_command(command, args.dry_run)

    print("\nGemma 3-12B suite finished.", flush=True)
