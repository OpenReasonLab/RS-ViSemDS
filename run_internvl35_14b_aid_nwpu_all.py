...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run InternVL3.5-14B zero-shot, random few-shot, and global RemoteCLIP "
            "kNN few-shot experiments on AID and NWPU-FG-Urban."
        )
    )
    ...
    parser.add_argument(
        "--model",
        default=os.environ.get("INTERNVL35_14B_MODEL", "/root/autodl-tmp/models/InternVL3.5-14B"),
    )
    parser.add_argument("--out-root", default="InternVL3.5-14B/results_eval100_seed42")
    ...
    for dataset in args.datasets:
        ...
        if not args.skip_zero:
            command = [
                ...
                *base_backend_args,
                "--out-dir", f"{args.out_root}/internvl35_14b_zero_{dataset}",
            ]
        ...
        for shot in args.shots:
            ...
            if not args.skip_random:
                ...
                command = [
                    ...
                    *base_backend_args,
                    "--out-dir", f"{args.out_root}/internvl35_14b_random_{dataset}_shot{shot}",
                ]
                run_command(command, args.dry_run)
            ...
            if not args.skip_knn:
                ...
                command = [
                    ...
                    *base_backend_args,
                    "--out-dir", f"{args.out_root}/internvl35_14b_knn_{dataset}_shot{shot}",
                ]
                run_command(command, args.dry_run)

    print("\nInternVL3.5-14B suite finished.", flush=True)
